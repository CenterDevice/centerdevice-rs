...
impl<'a> CliOAuth {
    ...
    pub fn get_code(self, extra_params: &mut Vec<(&str, String)>) -> Result<Code> {
        ...
        if self.open_browser {
            println!("Please authenticate in the web browser window, wait for the redirect, enter the code into the terminal, and then press return ...");
...
pub trait CodeWithBasicAuthSchemeResult {
    fn exchange_for_token<T: DeserializeOwned>(self, config: &Config) -> Result<T>;
}
...
impl CodeWithBasicAuthScheme {
    ...
    fn do_exchange_for_token<T: DeserializeOwned>(self, config: &Config) -> Result<T> {
        info!("Using the authentication code '{}'.", self.code.code);
        ...
        if config.general.output_format {
            info!("Received response: ");
            println!("{}", str::from_utf8(&buffer).chain_err(|| "Failed to print buffer")?);
        }
